...
class StoryRemoteMediator constructor(
...
) : RemoteMediator<Int, ResponseGetStory>(){
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, ResponseGetStory>
    ): MediatorResult {
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, ResponseGetStory>): RemoteKeys? {
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id)
        }
    }
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, ResponseGetStory>): RemoteKeys? {
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()?.let { data ->
            database.remoteKeysDao().getRemoteKeysId(data.id)
        }
    }
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int, ResponseGetStory>): RemoteKeys? {
